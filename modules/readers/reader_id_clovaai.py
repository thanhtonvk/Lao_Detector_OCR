...
# parser.add_argument('--saved_model', help="path to saved_model to evaluation",
#                     default="/home/daotuanvu/Downloads/best_norm_ED_lao_id_5.pth")
parser.add_argument('--saved_model', help="path to saved_model to evaluation",
                    default=config.MODEL_READER)
...
""" Data processing """
parser.add_argument('--batch_max_length', type=int, default=17, help='maximum-label-length')
...
print('opt.character', opt.character, len(opt.character))
...
def read(image_list, dictionary_list):
    ...
    with torch.no_grad():
        ...
        if 'CTC' in opt.Prediction:
            ...
            # print(preds.sort(2, descending=True))
            # select max probabilty (greedy decoding) then decode index to character
            _, preds_index = preds.max(2)
        ...
        # print(preds_prob.sort(2, descending=True))
