...
model_paths = {
    'hrnet18': 'models/detector/household_handwriting/front/pretrained-hrnet/hrnetv2_w18_imagenet_pretrained.pth',
    'hrnet32': 'models/detector/household_handwriting/front/pretrained-hrnet/hrnetv2_w32_imagenet_pretrained.pth',
    'hrnet48': 'models/detector/household_handwriting/front/pretrained-hrnet/hrnetv2_w48_imagenet_pretrained.pth'
}
...
def _hrnet(arch, pretrained, progress, **kwargs):
    from modules.hrnet.configs.configs import MODEL_CONFIGS
    model = HighResolutionNet(MODEL_CONFIGS[arch], **kwargs)
    if pretrained:
        model_path = model_paths[arch]
        logging.info('Loading pretrained model: ' + model_paths[arch])
        # state_dict = torch.load(model_path)
        state_dict = torch.load(torch_load_content(model_path))
