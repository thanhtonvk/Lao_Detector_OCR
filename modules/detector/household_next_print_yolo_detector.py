...
from modules.detector.detector_base import HouseholdNextPrintDetector
from entities.card_detection import (
    HouseholdNextPrintDetection,
    HouseholdNextPrintDetectionFields,
)
from modules.yolo.models.v5 import YoloModel
from utils.utils import scale_coords


class HouseholdNextPrintYoloDetector(HouseholdNextPrintDetector):
    model_weights = config.MODEL_HOUSEHOLD_NEXT_PRINT_DETECTOR
    img_size = 640
    conf_threshold = 0.5
    iou_threshold = 0.25
    v5 = True
    ...
    def detect_fields(self, cropped_image: np.ndarray) -> HouseholdNextPrintDetection:
        ...
        detection_fields = HouseholdNextPrintDetectionFields()
        for k in boxes:
            if len(boxes[k]) > 1:
                img_arr = []
                if k in [
        ...
        return HouseholdNextPrintDetection(content=detection_fields)
